...
describe('Event Endpoints', function() {
  ...
  const { testUsers, testEvent, testReviews } = helpers.makeEventFixtures();
  ...
  describe(`GET /api/things`, () => {
    ...
    context('Given there are things in the database', () => {
      beforeEach('insert things', () =>
        helpers.seedEventTables(db, testUsers, testEvent, testReviews)
      );

      it('responds with 200 and all of the things', () => {
        const expectedEvent = testEvent.map(thing =>
  ...
  describe(`GET /api/things/:thing_id`, () => {
    context(`Given no things`, () => {
      beforeEach('insert things', () => helpers.seedEventTables(db, testUsers));
